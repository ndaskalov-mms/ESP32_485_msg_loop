...
void printAlarmPartOpts(byte * PartArrPtr, int maxPart) {
  ...
  lprintf("      	      status armTime followZone2entryDelay2              follows\n");
	for (int j = 0; j < maxPart; j++) {
		lprintf ("Partitions status: %2d\t%d\t %u    ",(*prtArr)[j].armStatus, (*prtArr)[j].followZone2entryDelay2, (*prtArr)[j].armTime);
       for (int i = 0; i < MAX_PARTITION; i++) {                        
			  lprintf(" %d", (*prtArr)[j].follows[i]);
		}
		lprintf("\n");
  }
}
...
void printAlarmOpts(byte * optsPtr) {
	...
	lprintf("Alarm Global Options armRestrictions %d,  troubleLatch %d, tamperOpts %d, antiMaskOpt %d entryDelay1Start %u, entryDelay2Start %u\n",\
...
//
    ...
    for(int i = 0; i <= MAX_SLAVES; i++) {         // for each board 
        ...
        lprintf("Board %d alarm zones from zonesDB\n", i);
...
void setAlarmGlobalOptsDefaults() {
	...
	alarmGlobalOpts.antiMaskOpt = ZONE_TAMPER_OPT_DISABLED;		// follow zone settings for anti-mask
}
//
...
void setAlarmPartOptsDefaults() {
  ...
  lprintf("Setting random partititon follows\t");
  for(int i = 0; i < MAX_PARTITION; i++) {
...
void cleanRTdata() {
	 for(int i = 0; i < MAX_PARTITION; i++) {
		...
		partitionDB[i].entryDelayFSM = NOT_STARTED;
	 }
	 for(int i = 0; i <= MAX_SLAVES; i++) {         										// for each board 
        for(int j=0; j< (i?SLAVE_ALARM_ZONES_CNT:MASTER_ALARM_ZONES_CNT); j++) {            // for each board' zone
...
void initAlarm() {
   ...
   if(readConfig(configFileName))   {          //read config file  
  		lprintf("Copying from alarmConfig to zonesDB\n");
   ...
   lprintf("Setted alarm defaults for testing\n");
}
